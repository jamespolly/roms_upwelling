...
!
!git $Id$
!svn $Id: read_phypar.F 1184 2023-07-27 20:28:19Z arango $
!================================================== Hernan G. Arango ===
...
!
     ...
     &                      inp_lib, FRC)
          ...
          END SELECT
          IF (FoundError(exit_flag, NoError, 4223, MyFile)) RETURN
        END IF
      END DO
...
!
          ...
          IF (CLM_FILE(ng)) THEN
            DO ifile=1,CLM(i,ng)%Nfiles
              ...
              IF (.not.find_file(ng, out, fname, 'CLMNAME')) THEN
                IF (FoundError(exit_flag, NoError, 7488, MyFile))       &
     &            RETURN
              ELSE
                IF (ifile.eq.1) THEN
     ...
     &     (ANY(LnudgeTCLM(:,ng)))) THEN
          ...
          IF (.not.find_file(ng, out, fname, 'NUDNAME')) THEN
            IF (FoundError(exit_flag, NoError, 7507, MyFile)) RETURN
          ELSE
            IF (Master.and.Lwrite) WRITE (out,230)                      &
...
!
          ...
          DO i=1,nBCfiles(ng)
            DO ifile=1,BRY(i,ng)%Nfiles
              ...
              IF (.not.find_file(ng, out, fname, 'BRYNAME')) THEN
                IF (FoundError(exit_flag, NoError, 7549, MyFile))       &
     &            RETURN
              ELSE
                IF (ifile.eq.1) THEN
                  IF (Master.and.Lwrite) WRITE (out,310)                &
     ...
     &                                         TRIM(fname)
        ...
        IF (.not.find_file(ng, out, fname, 'VARNAME')) THEN
          IF (FoundError(exit_flag, NoError, 7605, MyFile)) RETURN
        ELSE
          IF (Master.and.Lwrite) WRITE (out,230)                        &
