...
!
!git $Id$
!svn $Id: get_state.F 1151 2023-02-09 03:08:53Z arango $
!================================================== Hernan G. Arango ===
...
!
      CALL wclock_on (ng, IDmod, 80, 277, MyFile)
!
...
!
      ...
      IF (FoundError(exit_flag, NoError, 306, MyFile)) THEN
...
!
     ...
     &                          OCEAN(ng) % zeta(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 657, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % ubar(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 784, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        Fscl, Fmin, Fmax,                         &
     &                        OCEAN(ng) % rvbar)
            IF (FoundError(status, nf90_noerr, 964, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % v(:,:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 1157, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                            OCEAN(ng) % t(:,:,:,Tindex,itrc))
              END IF
              IF (FoundError(status, nf90_noerr, 1281, MyFile)) THEN
                IF (Master) THEN
...
!
      CALL wclock_off (ng, IDmod, 80, 7339, MyFile)
!
